...
for i=1:numel(simulations)
    for j=1:Nb_seism
        ...
        fprintf('\n [%s] a = %d, b = %d', simulations{i}, abs(s{i,j}(882,6)-s{1,j}(882,6)), max(m{1,j}(range,4)))
     end
end
...
for seism=1:Nb_seism
    ...
    drawnow
    print('-depsc','-r300',sprintf('figures/Fig_16%s',letters(seism)))
...
%         ax.YLim = [a_ylim b_ylim];
    ...
    set(gcf,'Position',[577,282,911,663]);
    drawnow
    print('-depsc','-r300',sprintf('figures/Fig_17%s',letters(seism)))
