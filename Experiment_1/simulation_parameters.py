...
# Wave speeds
c1p = params.V1.vector().max()
c2p = params.V2.vector().min()
...
# Wave lengths
omega_h = omega_c + omega_b             # Frequency to evaluate
omega_l = omega_c - omega_b             # Frequency to evaluate
...
P      = 1                            # Order of the Lagrange polynomials
n      = 12                           # Number of elements per wavelength
cell_d = P*(Ls/n)                     # Maxmimum cell diameter
CFL    = 0.75                         # CFL number
...
print('\n[*] Wave velocities')
print('   Fast primary wave speed (c1p):  {:.2f} [m/s]'.format(c1p))
print('   Slow primary wave speed (c2p):  {:.2f} [m/s]'.format(c2p))
print('   Secondary wave speed (cs):      {:.2f} [m/s]'.format(cs))
